...
2. **どちらか**:
   ...
   - **B**: `gh pr create --base main --title "feat: …" --body-file .github/pull_request_template.md` で、エディタで各セクションを埋めてから保存（テンプレの HTML コメントは説明用・GitHub 上では非表示）。
3. ブラウザでプレビューし、箇条書きとチェックが揃っているか確認。

エージェントが PR を作るときは、**テンプレの見出しごとに**概要・変更点・テスト・関連を埋めた `--body` を渡すか、`--body-file` 用に一時ファイルへ整形してから `gh pr create` すること。
...
1. **PR の有無とベースブランチを取得**
   ...
   エラーなら PR なし → 新規作成フローへ。成功なら `baseRefName`（例: `main`）を控える。
